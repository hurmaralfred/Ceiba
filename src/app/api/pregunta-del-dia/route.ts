...
// 200+ preguntas organizadas en 10 ejes. Si Claude no está disponible, el motor
// rota por ejes garantizando que no se repita el mismo eje dos días seguidos
// y que el pool dure ~200 días sin repetir.

const QUESTION_POOL: { theme: string; questions: string[] }[] = [
  {
    theme: "infancia",
    questions: [
  ...
  },
  {
    ...
    questions: [
      ...
      "¿Hay alguien en tu familia que haya roto una barrera o hecho algo que nadie en la familia había hecho antes?",
...
// Número total de preguntas en el pool
const TOTAL_QUESTIONS = QUESTION_POOL.reduce((s, t) => s + t.questions.length, 0);

// Eje temático del día — rota sin repetir el mismo dos días seguidos
function getDayTheme(dayNumber: number): { theme: string; questions: string[] } {
  const idx = dayNumber % QUESTION_POOL.length;
  return QUESTION_POOL[idx];
}

// Pregunta de fallback: rota dentro del eje del día usando el spaceHash
function getFallbackQuestion(dayNumber: number, spaceHash: number): string {
  const pool = getDayTheme(dayNumber);
  const qIdx = (dayNumber + spaceHash) % pool.questions.length;
  return pool.questions[qIdx];
}
...
async function getFamilyContext(
  ...
  if (persons) {
    ...
    const upcoming = (persons as any[]).filter((p) => {
    ...
    if (upcoming.length > 0)
      parts.push(`cumpleaños próximos: ${upcoming.map((p: any) => p.first_name).join(", ")}`);
  }
...
async function generateWithClaude(
  ...
  const themeLabel = themeLabels[dayTheme] ?? "dinámicas familiares cotidianas";
  const avoidBlock =
    recentQuestions.length > 0
      ? `\n\nPreguntas que ya se han hecho (NO repitas ni el tema ni el enfoque de ninguna):\n${recentQuestions.map((q, i) => `${i + 1}. ${q}`).join("\n")}`
      : "";
...
Eje temático del día: ${themeLabel}${avoidBlock}

Genera UNA pregunta reflexiva, emotiva y específica sobre el eje temático indicado.
La pregunta debe:
- Hablar de dinámicas familiares de la vida cotidiana real
- Ser diferente en enfoque y emoción a las preguntas ya hechas
- Invitar a un recuerdo concreto, una historia o una emoción genuina
- Ser directa, sin rodeos, en español natural latinoamericano
...
export async function GET() {
  ...
  // Sin espacio familiar: fallback global sin cache
  if (!spaceId) {
    const fallback = getDayTheme(dayNumber).questions[dayNumber % getDayTheme(dayNumber).questions.length];
    return NextResponse.json({ question: fallback, total_pool: TOTAL_QUESTIONS });
  }

  // Chequear cache del día
  ...
  if ((cached as any)?.question_text) {
    return NextResponse.json({ question: (cached as any).question_text, total_pool: TOTAL_QUESTIONS });
  }

  // Obtener historial y contexto en paralelo
