...
describe('AvatarFigure — photo error reset on URL change', () => {
  it('resets photoError when avatarUrl changes to a new URL', async () => {
    const url1 = 'https://example.com/broken.jpg'
    const url2 = 'https://example.com/ok.jpg'
    const { container, rerender } = render(
      <AvatarFigure node={makeNode({ avatarUrl: url1 })} />,
    )

    // Trigger error on url1
    const imgEl = container.querySelector('image[href]')
    await act(async () => { fireEvent.error(imgEl!) })

    // Now face renders (photo errored)
    expect(container.querySelector('image[href]')).toBeNull()

    // Change to a different URL — error must reset so photo renders again
    await act(async () => {
      rerender(<AvatarFigure node={makeNode({ avatarUrl: url2 })} />)
    })

    expect(container.querySelector('image[href]')).not.toBeNull()
  })

  it('does not share photoError state between different persons', async () => {
    const broken = 'https://example.com/broken.jpg'

    const { container: c1 } = render(<AvatarFigure node={makeNode({ id: 'p1', avatarUrl: broken })} />)
    const { container: c2 } = render(<AvatarFigure node={makeNode({ id: 'p2', avatarUrl: broken })} />)

    // Trigger error only on p1
    await act(async () => { fireEvent.error(c1.querySelector('image[href]')!) })

    // p1 falls back to SVG — photo removed
    expect(c1.querySelector('image[href]')).toBeNull()
    // p2 still shows its photo (independent state)
    expect(c2.querySelector('image[href]')).not.toBeNull()
  })
})
